...
from models.yolov2_d19 import train_output, eval_output, create_grid
from models.yolov2_d19 import YOLOv2D19_QAT
import quantization
...
def main():
    ...
    all_dataset = VOCDetection(data_dir=data_dir, 
    ...
    QAT_model = train(net, args, cfg, device='cpu')
    QAT_model.to(device)
    print('\nAfter QAT:')
    print_scale_and_zp(QAT_model, 'activation_post_process_0')
    voc_test(QAT_model, data_dir, device, input_size)
    ...
    # load net
    net.load_state_dict(torch.load(args.trained_model, map_location='cuda'))
    net = quantization.fuse_conv_bn(net)
    net.load_state_dict(QAT_model.state_dict(), strict=False)


    arch = 'YOLOV2'

    int8_model = quantization.auto_quant(arch, QAT_model, train_dataset, precision='INT8', acc_loss = 0.01, mode = 'PTQ', device=device, useConv2D=True, useSmooth=False)
...
def train(net, args, cfg, device):
    ...
    example_inputs = (torch.randn(args.input_size),)
    model.eval()
    ...
    for epoch in range(args.start_epoch, max_epoch):
        ...
        for iter_i, (images, targets) in enumerate(dataloader):
            ...
            # forward
            pred = model(images, target=targets)
            conf_loss, cls_loss, box_loss, iou_loss = train_output(pred, targets, train_size)
        ...
        if distributed_utils.is_main_process():
            ...
            if (epoch % args.eval_epoch) == 0 or (epoch == max_epoch - 1):
                ...
                if evaluator is None:
                    ...
                    model_eval.input_size = val_size
                    model_eval.grid_cell, model_eval.all_anchor_wh = create_grid(val_size)
                    # model_eval.set_grid(val_size)
                    model_eval.eval()

                    # evaluate
                    evaluator.evaluate(model_eval, val_size)
                    ...
                    model_eval.input_size = train_size
                    model_eval.grid_cell, model_eval.all_anchor_wh = create_grid(train_size)
